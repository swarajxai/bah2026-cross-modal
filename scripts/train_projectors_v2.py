"""
Train v2 projection heads with the v2 combined loss on CPU.

This is the local equivalent of the Colab training pipeline — uses
ModalityProjector (3-layer + BN + residual + class-conditional LN)
and CombinedLoss (Triplet + InfoNCE + CrossModal + Prototype).

Key improvements over v1:
  - Stronger loss (4 components vs 2)
  - Class-conditional layer norm (per-class affine)
  - Cosine LR schedule with warmup
  - 25 epochs vs 12
  - Larger hidden dim (1024 vs 512)

Saves: outputs/projector.pt (auto-detected as v2 by the Flask app)
"""
...
def train(features_npz: str, output_ckpt: str,
          ...
          triplet_w: float = 0.25, nce_w: float = 0.35,
          cm_w: float = 0.25, proto_w: float = 0.15,
          seed: int = 42, dropout: float = 0.15,
          warmup_epochs: int = 2):
    print(f"[train-v2] loading features from {features_npz}")
    ...
    print(f"[train-v2] emb shape: {emb.shape}, classes: {n_classes}, mods: {set(mods)}")
    print(f"[train-v2] hidden_dim={hidden_dim}, dropout={dropout}, lr={lr}, epochs={epochs}")
    ...
    print(f"\n[train-v2] starting {epochs} epochs ...\n")
    t0_total = time.time()
    ...
    torch.save({"feat_dim": feat_dim, "hidden_dim": hidden_dim, "out_dim": out_dim,
                ...
                "n_classes": n_classes}, output_ckpt)
    print(f"\n[train-v2] saved -> {output_ckpt}  (best_loss={best_loss:.4f}, total {total_dt:.0f}s)")
