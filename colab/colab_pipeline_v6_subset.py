# =====================================================================
#  BAH 2026 — DINOv2 V6 Training (Local Subset Edition — FASTEST)
# =====================================================================
#
#  Colab pe upload karne wala ONE-CELL script.  Pre-built subset use
#  karta hai (300 files per class/modality = ~500 MB) for FAST upload.
#  HF ki zarurat nahi, Drive ki zarurat nahi.
#
#  Quick start:
#    1. Open Colab, T4 GPU
#    2. ONE cell me paste karo ye file
#    3. Upload D:\BAH2026_colab_subset.zip (download karo main script se)
#    4. ~30 min wait → auto-download v6 artifacts
# =====================================================================

# ===================== 0. CONFIG =====================
INSTALL = True
SUBSET_ZIP_NAME = "BAH2026_colab_subset.zip"
...
# Upload subset zip
print(f"\n[upload] upload {SUBSET_ZIP_NAME} (~500 MB, ~5 min)...")
from google.colab import files
uploaded = files.upload()
zip_name = list(uploaded.keys())[0]
print(f"[upload] got {zip_name}, unzipping...")

!mkdir -p /content/BAH2026
!unzip -q -o "{zip_name}" -d /content/
print("[upload] done. checking structure...")
!ls /content/BAH2026/
...
print(f"[scan] EuroSAT: {len(eurosat)} pairs, Sentinel: {len(sent_paired)} paired")
...
if Path(FEATURES_PATH).exists():
    ...
    for i in tqdm(range(len(items)), desc="extract DINOv2 (TTA x4)"):
        ...
        if (i+1) % 200 == 0: print(f"[extract] {i+1}/{len(items)}  {time.time()-t0:.0f}s", flush=True)
