# =====================================================================
#  BAH 2026 — DINOv2 V6 Training (Drive Edition)
# =====================================================================
#
#  Single-script Colab pipeline (Drive version).
#
#  Quick start:
#    1. Upload `D:\BAH2026_data.zip` to Google Drive root (NOT a folder)
#       OR: upload the unzipped `EuroSAT/` + `Sentinel/` folders to Drive
#    2. Open Colab, T4 GPU
#    3. Paste this whole file into ONE cell
#    4. Run
#    5. Auto-downloads `cross_modal_v6.zip` at the end
#
#  This is the Drive edition — works WITHOUT re-uploading each session.
# =====================================================================

# ===================== 0. CONFIG =====================
INSTALL = True            # first run me True rakhna hai (baad me False kar sakte ho)
DRIVE_ROOT = "/content/drive/MyDrive"

# Data location — choose ONE:
#   Option A (recommended): zip uploaded to Drive root, we unzip inside Colab
#   Option B:               folder already unzipped in Drive
USE_DRIVE_ZIP = True
DATA_ZIP_NAME = "BAH2026_data.zip"   # must be at Drive root
# If unzipping manually instead:
DATA_FOLDER_NAME = "BAH2026"         # contains EuroSAT/ + Sentinel/
...
# Output paths (inside Colab VM, NOT Drive — fast I/O)
...
print(f"[setup] Drive mounted at {DRIVE_ROOT}")
!ls -la "{DRIVE_ROOT}/" 2>/dev/null | head -20

# ===================== 2. LOCATE / UNZIP DATA =====================
print("\n[setup] locating data ...")
BASE_DIR = f"{DRIVE_ROOT}/{DATA_FOLDER_NAME}"
zip_on_drive = f"{DRIVE_ROOT}/{DATA_ZIP_NAME}"

if USE_DRIVE_ZIP and not Path(BASE_DIR).exists():
    if Path(zip_on_drive).exists():
        print(f"[setup] found zip at {zip_on_drive}, unzipping ...")
        # Fast: unzip in /content (VM-local SSD) — 5x faster than Drive I/O
        !mkdir -p /content/BAH2026
        t0 = time.time()
        !unzip -q -o "{zip_on_drive}" -d /content/
        print(f"[setup] unzipped in {time.time()-t0:.0f}s")
        BASE_DIR = "/content/BAH2026"
    else:
        print(f"[setup] ERROR: {zip_on_drive} not found.")
        print(f"[setup] Upload {DATA_ZIP_NAME} to your Drive root, then re-run.")
        sys.exit(1)
elif Path(BASE_DIR).exists():
    print(f"[setup] using pre-unzipped folder: {BASE_DIR}")
else:
    print(f"[setup] ERROR: no data found at {BASE_DIR}")
    sys.exit(1)

print(f"[setup] final BASE_DIR = {BASE_DIR}")
!ls "{BASE_DIR}/"
...
print(f"[scan] EuroSAT pairs: {len(eurosat)}, Sentinel paired: {len(sent_paired)}, unpaired: {len(sent_unpaired)}")
